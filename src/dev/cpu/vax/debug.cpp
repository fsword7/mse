...
int vax_cpuDevice::listOperand(uint32_t &vAddr, const vaxOpcode *opc, char **ptr, uint32_t &nWords)
{
	int      opMode, opScale;
	uint32_t opType;
	uint32_t data;
...
int vax_cpuDevice::list(const cty_t *cty, uint32_t vAddr)
{
	...
	if (opc->opCode != OPC_nUOPC) {
		...
		listOperand(pcAddr, opc, &ptr, nWords);
	} else {
		if (opCode > OPC_nEXTEND)
